...
type ValidateCodeSentBySMSProps = {
  userToken: string
  userPassword: string
  xAuthToken: string
}

export class ValidateCodeSentBySMSProvider implements ApplicationProvider {
  ...
  async execute({
    ...
    userPassword,
    xAuthToken
  }: ValidateCodeSentBySMSProps): Promise<
  ...
  > {
    ...
    const _endpoint = `${this.applicationHttpRequest.getBaseURL()}/user/cellphone/validate`
