...
type GetUserCellPhoneDataProps = {
  applicationToken: string
  uniqueIdentifier: string
  xAuthToken: string
}

export class GetUserCellPhoneDataProvider implements ApplicationProvider {
  ...
  async execute({
    ...
    uniqueIdentifier,
    xAuthToken
  }: GetUserCellPhoneDataProps): Promise<
  ...
  > {
    ...
    const _queryString = `ApplicationToken=${applicationToken}&UniqueIdentifier=${uniqueIdentifier}`
    const _endpoint = `${this.applicationHttpRequest.getBaseURL()}/user/cellphone?${_queryString}`
