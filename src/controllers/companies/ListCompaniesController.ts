...
export default class ListCompaniesController implements HTTPRequest {
  private listCompanyService: ApplicationService

  constructor(listCompanyService: ApplicationService) {
  ...
  async handle({
    ...
    try {
      ...
      const result = await this.listCompanyService.run({
        search,
        page: skip,
        perPage: take
      })
