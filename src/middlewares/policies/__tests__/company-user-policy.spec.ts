/* eslint-disable max-len */
/* eslint-disable @typescript-eslint/no-explicit-any */
import { Request } from 'express'
import { CompanyUserPolicy } from '../CompanyUserPolicy'
import Admin from '../../../entities/Admin'
import { ApplicationError } from '../../../helpers/ApplicationError'
import { unauthorized } from '../../../adapters/http/HttpResponseAdapter'
import adminMock from '../../../__mocks__/entities/admin-mock'
import companyUserMock from '../../../__mocks__/entities/company-user-mock'
import CompanyUser, { Role } from '../../../entities/CompanyUser'
import { t } from '../../../config/i18next/I18nextLocalization'

describe('CompanyPolicy', () => {
  describe('When the user can access company features', () => {
    it('should return 401 unauthorized if user is not provided', async () => {
      const policy = new CompanyUserPolicy()
      const request = {} as Request
      const response = {
        status: jest.fn().mockReturnThis(),
        send: jest.fn()
      } as any
      const next = jest.fn()

      await policy.canCreateCompanyUser()(request, response, next)

      expect(response.status).toHaveBeenCalledWith(401)
      expect(response.send).toHaveBeenCalledWith(
        unauthorized(new ApplicationError(t('error.unauthorized')))
      )
      expect(next).not.toHaveBeenCalled()
    })

    it('should return 401 unauthorized if user is not an instance of Admin or CompanyUser owner', async () => {
      const policy = new CompanyUserPolicy()
      const request = { user: {} } as Request
      const response = {
        status: jest.fn().mockReturnThis(),
        send: jest.fn()
      } as any
      const next = jest.fn()

      await policy.canCreateCompanyUser()(request, response, next)

      expect(response.status).toHaveBeenCalledWith(401)
      expect(response.send).toHaveBeenCalledWith(
        unauthorized(new ApplicationError(t('error.unauthorized')))
      )
      expect(next).not.toHaveBeenCalled()
    })

    it('should call next() if user is an instance of Admin', async () => {
      const policy = new CompanyUserPolicy()
      const adminUser = new Admin(adminMock)
      const request = { user: adminUser } as any
      const response = {
        status: jest.fn().mockReturnThis(),
        send: jest.fn()
      } as any
      const next = jest.fn()

      await policy.canCreateCompanyUser()(request, response, next)

      expect(response.status).not.toHaveBeenCalled()
      expect(response.send).not.toHaveBeenCalled()
      expect(next).toHaveBeenCalled()
    })

    it('should call next() if user is an instance of CompanyUser and has the "owner" role', async () => {
      const policy = new CompanyUserPolicy()
      const companyUser = new CompanyUser({
        ...companyUserMock,
        role: Role.owner
      })
      const request = { user: companyUser } as any
      const response = {
        status: jest.fn().mockReturnThis(),
        send: jest.fn()
      } as any
      const next = jest.fn()

      await policy.canCreateCompanyUser()(request, response, next)

      expect(response.status).not.toHaveBeenCalled()
      expect(response.send).not.toHaveBeenCalled()
      expect(next).toHaveBeenCalled()
    })

    it('should return 401 unauthorized if user is an instance of CompanyUser but not an owner', async () => {
      const policy = new CompanyUserPolicy()
      const companyUser = new CompanyUser({
        ...companyUserMock,
        role: Role.manager
      })
      const request = { user: companyUser } as any
      const response = {
        status: jest.fn().mockReturnThis(),
        send: jest.fn()
      } as any
      const next = jest.fn()

      await policy.canCreateCompanyUser()(request, response, next)

      expect(response.status).toHaveBeenCalledWith(401)
      expect(response.send).toHaveBeenCalledWith(
        unauthorized(new ApplicationError(t('error.unauthorized')))
      )
      expect(next).not.toHaveBeenCalled()
    })
  })
})
