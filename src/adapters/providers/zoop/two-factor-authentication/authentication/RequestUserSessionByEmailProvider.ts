...
type RequestUserSessionByEmailProps = {
  userToken: string
  userPassword: string
  xAuthToken: string
}

export class RequestUserSessionByEmailProvider implements ApplicationProvider {
  ...
  async execute({
    ...
    userPassword,
    xAuthToken
  }: RequestUserSessionByEmailProps): Promise<
  ...
  > {
    ...
    const _endpoint = `${this.applicationHttpRequest.getBaseURL()}/user/email/session`
