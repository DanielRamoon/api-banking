...
type RequestUserSessionBySMSProps = {
  userToken: string
  userPassword: string
  xAuthToken: string
}

export class RequestUserSessionBySMSProvider implements ApplicationProvider {
  ...
  async execute({
    ...
    userPassword,
    xAuthToken
  }: RequestUserSessionBySMSProps): Promise<
  ...
  > {
    ...
    const _endpoint = `${this.applicationHttpRequest.getBaseURL()}/user/cellphone/session`
