...
export default class ListUsersController implements HTTPRequest {
  private listUserService: ApplicationService

  constructor(listUserService: ApplicationService) {
  ...
  async handle({
    ...
    page,
    per_page
  }: ListUsersParams): Promise<HttpResponse> {
    try {
      const skip = parseInt(page ?? '0')
      const take = parseInt(per_page ?? '10')
      search = search ?? ''

      const result = await this.listUserService.run({
